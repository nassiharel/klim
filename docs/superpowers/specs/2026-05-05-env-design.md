...
4. Registers custom packs via `custompacks.Add` (existing packs with the same name are preserved; users delete from `~/.klim/marketplace/custom-packs.yaml` or via the TUI to replace).
