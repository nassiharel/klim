...
type assembledFile struct {
	...
	AgentMarketplaces []registry.AgentMarketplaceDef `yaml:"agent_marketplaces,omitempty"`
}
...
func main() {
	...
	// --- Load agent marketplaces ---
	amFiles, err := filepath.Glob(filepath.Join(*dir, "agent-marketplaces", "*.yaml"))
	if err != nil {
		fatal("globbing agent-marketplace files: %v", err)
	}

	var agentMarketplaces []registry.AgentMarketplaceDef
	for _, f := range amFiles {
		...
		agentMarketplaces = append(agentMarketplaces, am)
	}

	sort.SliceStable(agentMarketplaces, func(i, j int) bool {
		return strings.ToLower(agentMarketplaces[i].Name) < strings.ToLower(agentMarketplaces[j].Name)
	})
	...
	// --- Assemble ---
	assembled := assembledFile{Tools: tools, Packs: packs, AgentMarketplaces: agentMarketplaces}
