...
Usage:
  ...
  powershell:
	...
	Args:                  cobra.MatchAll(requireArgs(1, "klim shell hook <bash|zsh|fish|powershell>"), cobra.OnlyValidArgs),
...
const hookBash = `# klim shell hook — auto-check .klim.yaml on cd
__clim_hook() {
...
const hookZsh = `# klim shell hook — auto-check .klim.yaml on cd
...
const hookFish = `# klim shell hook — auto-check .klim.yaml on cd
...
const hookPowerShell = "# klim shell hook \u2014 auto-check .klim.yaml on cd\n" +
