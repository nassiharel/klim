...
// Write writes the manifest to path. Atomic (temp file + rename) so an
// overwrite or interrupted write can't leave a half-written .clim.yaml
// in a shared repo.
func Write(tf *TeamFile, path string) error {
	...
	return fileutil.AtomicWrite(path, []byte(header+string(data)), 0o644)
}
