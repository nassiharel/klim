...
type Model struct {
	...
	trailState      string          // trailViewIdle / trailViewLabelInput
	trailEntries    []trail.Entry   // loaded entries (nil = not loaded yet)
	trailCursor     int             // selected row in the list
	trailError      string          // last error message
	trailLabelInput textinput.Model // text-input for capture label
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case envBuildResultMsg:
		// Local env profile build completed. Update the sub-view
		// fields so the next render shows the token + metadata.
		if msg.err != nil {
			m.envError = msg.err.Error()
			m.statusMsg = ""
			return m, nil
		}
		m.envProfile = msg.profile
		m.envToken = msg.token
	...
	case envDecodedMsg:
		if msg.err != nil {
			m.envError = "decode failed: " + msg.err.Error()
			m.envState = envViewIdle
			m.statusMsg = ""
			return m, nil
		}
		m.envRemoteProfile = msg.profile
		m.envError = ""
		switch msg.verb {
		case "open":
			m.envShowText = formatEnvShow(msg.profile)
			m.envState = envViewShowResult
		case "diff":
			if m.envProfile == nil {
				m.envError = "local profile not built yet — try refresh (r) first"
				m.envState = envViewIdle
				return m, nil
			}
			m.envDiffText = formatEnvDiff(m.envProfile, msg.profile)
			m.envState = envViewDiffResult
		case "apply":
	...
	case envApplyResultMsg:
		// Favorites + custom packs side-effects finished; assemble
		// the report so the user sees what changed beyond the
		// per-tool installs that already rendered through the
		// backupItems progress bar.
		var b strings.Builder
		if msg.err != nil {
			b.WriteString("⚠ " + msg.err.Error() + "\n")
		}
		fmt.Fprintf(&b, "Favorites: %d added (of %d in env)\n", msg.favoritesAdded, msg.favoritesTotal)
		fmt.Fprintf(&b, "Custom packs: %d added (of %d in env)\n", msg.packsAdded, msg.packsTotal)
		m.envApplyReport = b.String()
		...
		// rescan.
		if names, err := favorites.Load(); err == nil {
			m.favoriteNames = make(map[string]bool, len(names))
			for _, n := range names {
				m.favoriteNames[n] = true
			}
		}
		return m, nil

	case vulnScanResultMsg:
		// Forced OSV scan finished. The cache file is already
		// updated; we just need to refresh the TUI's derived
		// state so the Security badges and detail-view section
		// pick up the new data without a tab-toggle dance.
		if msg.err != nil {
			m.statusMsg = "⚠ Vuln scan failed: " + msg.err.Error()
			return m, nil
		}
		count := 0
		if msg.report != nil {
			for _, mt := range msg.report.Matches {
				count += len(mt.Vulnerabilities)
			}
		}
		m.statusMsg = fmt.Sprintf("✓ Vuln scan complete (%d findings)", count)
		// Re-evaluate compliance with the fresh severity data and
		// recompute audit/dashboard so the changed counts flow
		// through the score gauge.
		m.recomputeComplianceDerivedState()
		return m, nil
	...
	case trailLogResultMsg:
		if msg.err != nil {
			m.trailError = msg.err.Error()
			m.trailEntries = nil
			m.statusMsg = ""
			return m, nil
		}
		m.trailEntries = msg.entries
		m.trailError = ""
		if m.trailCursor >= len(m.trailEntries) {
			m.trailCursor = 0
		}
		m.statusMsg = fmt.Sprintf("✓ %d entries", len(msg.entries))
		return m, nil

	case trailCaptureResultMsg:
		if msg.err != nil {
			m.trailError = msg.err.Error()
			m.statusMsg = "⚠ Capture failed"
			return m, nil
		}
	...
	case backupItemDoneMsg:
		...
		m.statusMsg = m.importSummary()
		cmd := m.startScan()
		if m.envApplyPending && m.envApplyProfile != nil {
			p := m.envApplyProfile
			m.envApplyPending = false
			m.envApplyProfile = nil
			return m, tea.Batch(cmd, applyEnvSideEffectsCmd(p))
		}
		return m, cmd
...
func (m Model) handleKeyDefault(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	if m.activeTab == tabDashboard || m.activeTab == tabDoctor {
		...
		case "v":
			...
			return m, nil
		case "R":
			// Capital R force-refreshes the compliance policy
			// from compliance.url. Scoped to the Compliance
			// sub-tab; lowercase r is already "rescan tools".
			if m.activeTab == tabDoctor && m.doctorSubTab == doctorSubCompliance {
				if m.cfg == nil || m.cfg.Compliance.URL == "" {
					m.statusMsg = "⚠ compliance.url not configured"
					return m, nil
				}
				m.statusMsg = "Refreshing compliance policy..."
				return m, refreshCompliancePolicyCmd(m.cfg)
			}
			return m, nil
		}
		return m, nil
