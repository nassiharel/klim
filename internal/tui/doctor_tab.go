...
func (m Model) renderComplianceView() string {
	...
	if m.complianceResult == nil {
		...
		// Highlight the in-TUI shortcut. The CLI command stays available
		// for users on a server / over SSH where the TUI isn't running.
		b.WriteString("  " + doctorSection.Render("Press i") + " " + dashDim.Render("to generate a sample policy at the default location.") + "\n")
		b.WriteString("  " + dashDim.Render("Equivalent CLI command: klim security compliance init") + "\n")
...
// runComplianceInit creates the sample compliance policy at the default
// location and refreshes the compliance sub-tab so the new policy takes
// effect immediately. Mirrors `klim security compliance init` minus the
// CLI's --policy override (the TUI has no per-invocation flags) and
// surfaces the outcome through statusMsg so the result is visible
// regardless of which sub-tab the user lands on next.
func (m *Model) runComplianceInit() {
	path, err := paths.CompliancePolicy()
	if err != nil {
		m.statusMsg = "⚠ Cannot resolve compliance policy path: " + err.Error()
		return
	}
	if err := compliance.WriteSamplePolicy(path); err != nil {
		// Most common failure here is "already exists" when a config
		// race lets the file appear between empty-state render and key
		// press. Surface the underlying error verbatim — it's already
		// human-readable.
		m.statusMsg = "⚠ " + err.Error()
		return
	}
	// Reload from the new file. We pass policyPath="" so the standard
	// resolution order applies; URL/Policy cfg overrides still win when
	// the user has them set, in which case the just-written file is
	// shadowed and the UI shows the configured source instead.
	...
	m.statusMsg = "✓ Sample compliance policy created at " + path
}
